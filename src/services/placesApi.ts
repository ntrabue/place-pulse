import { SearchState } from "../state/place-search-state";
...
async function geocodeLocation(
  city: string,
  state: string,
  country = "USA" // not using this yet but would be easy to support other countries
): Promise<{ lat: number; lng: number }> {
  const response = await fetch(
    `${API_BASE_URL}/api/geocode?address=${encodeURIComponent(
      `${city}, ${state}, ${country}`
    )}`
  );
...
function getPhotoUrl(photoName: string, maxWidth: number = 400): string {
  return `${API_BASE_URL}/api/places/photo?photoName=${encodeURIComponent(
    photoName
  )}&maxWidth=${maxWidth}`;
}
...
function transformToBusiness(place: any, details?: any): Business {
  ...
  return {
    ...
    userRatingsTotal:
      data.userRatingCount || place.userRatingCount || data.user_ratings_total,
    ...
    businessStatus:
      data.businessStatus || place.businessStatus || data.business_status,
    ...
    openingHours:
      data.regularOpeningHours || data.opening_hours
        ? {
            openNow:
              data.regularOpeningHours?.openNow || data.opening_hours?.open_now,
            weekdayText:
